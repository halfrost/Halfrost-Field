...
title = "Ubiquitous Digital Signatures"
...
From the previous article, we learned that a message authentication code can detect tampering or whether the sender’s identity has been impersonated—that is, it verifies message integrity and can also authenticate the message. However, the weakness of a message authentication code lies in its shared key. Because the key is shared, it cannot prevent repudiation.

Digital signatures are designed to solve the problem of repudiation. The approach is to ensure that the two communicating parties do not use the same shared key, so that the keys themselves can distinguish who is who.
...
A digital signature is a technology that implements the equivalent of sealing or signing in the real world within the computer world. Digital signatures can detect tampering, detect impersonation, and prevent repudiation.

There are two kinds of operations in digital signatures:

- Generating a signature for a message
- Verifying a signature for a message

The person who **generates the message signature** is the message sender; this is also called “signing the message.” Generating a signature means computing the value of the digital signature based on the message content.

The person who **verifies the digital signature** is a third party. The third party verifies whether the message originated from the sender. The verification result can be either success or failure.

**Digital signatures distinguish between the signing key and the verification key. A verification key cannot be used to generate a signature**. The signing key can only be held by the signer, while the verification key can be held by anyone who needs to verify the signature.
...
|Who holds the key?|Held by an individual|Anyone can hold it as needed|

Strictly speaking, in the RSA algorithm, public-key encryption and digital signatures are exact opposites. However, in other public-key algorithms, the relationship with digital signatures may not be exactly opposite.
...
There are two ways to generate and verify digital signatures:
...
Comparing the two methods above, the second method is generally used. The reason is that the first method must encrypt the entire message, and because it uses a public-key algorithm, it is very time-consuming. Using a short one-way hash function as a substitute for the message itself, and then encrypting it (signing the message), is much faster.

Here are some common questions about the two methods above:

- Why can encrypted ciphertext have the meaning of a signature?
	- Digital signatures are implemented by leveraging the property that “someone without the private key cannot generate ciphertext produced using that private key.” The generated ciphertext is not used to ensure confidentiality; instead, it represents **information that can only be generated by the holder of that key**. Therefore, ciphertext produced by a private key is a kind of **authenticator**.

- In method 2 above, the message is sent directly without being encrypted. Doesn’t that mean message confidentiality cannot be guaranteed?
...
- Can a signature be copied freely?
	- The meaning represented by a digital signature is that **a specific signer is bound to a specific message**. Although a digital signature can be copied freely, the meaning it represents never changes.

- If someone extracts the signature and combines it with an arbitrary message, couldn’t that forge the signer’s intent?
	- Digital signatures detect modification. During verification, the verifier will find that the hash value of the message does not match the signature, causing verification to fail and the message to be discarded.

- Is it possible to perform a collision attack by modifying both the message and the signature to deceive the verifier?
	- In practice, this cannot be done. First, once the message is modified, its hash value changes. Trying to assemble a valid signature afterward is not feasible, because without the private key, the new hash value cannot be encrypted.

- Can you go back on a digital signature after signing?
	- Digital signatures are inherently used to prevent repudiation. Once signed, they cannot be repudiated, and the contract cannot be torn up. The only option is to create another declaration message stating that the public key has been revoked, and add another digital signature to that message.
...
To ensure that downloaded software is safe and not a malicious virus, the software author adds a digital signature to the software. After the user finishes downloading it, verifying the digital signature can determine whether the downloaded software has been tampered with.

### 3. Public-Key Certificates

A legitimate public key is required when verifying a digital signature, but how can you know whether the public key you obtained is legitimate? In this case, the public key itself needs to be treated as information and signed with a digital signature, producing a public-key certificate. Certificate-related issues will be analyzed in detail in the next article.
...
```c
Signature = Message^D mod N (generate signature with RSA)
```
The D and N above are the signer’s private key. A signature is the result of raising the message to the Dth power modulo N.


### 2. Verifying Signatures with RSA
```c
message recovered from signature = signature^E mod N (verify signature with RSA)
```
The E and N above are the signer's public key. The verifier raises the signature to the power of E and takes mod N, obtaining the “message derived from the signature.” The verifier then compares this message with the message sent directly by the sender. If they match, verification succeeds; otherwise, it fails.
...
The ElGamal scheme is a public-key algorithm designed by Taher ElGamal. It relies on the difficulty of computing discrete logarithms modulo N. The ElGamal scheme can be used for both public-key encryption and digital signatures.


### 2. DSA

DSA (Digital Signature Algorithm) is a digital signature algorithm. It is part of the Digital Signature Standard (DSS), defined by NIST (National Institute of Standards and Technology) in 1991. DSA is a variant of the Schnorr algorithm and the ElGamal scheme. It can only be used for digital signatures, not for encryption and decryption.

Similar to the DH algorithm, the key thing to understand about DSA is its parameters. A key pair is generated from a parameter file. p, q, and g are public parameters, and key pairs are generated from them. DSA's public parameters are very similar to DH's public parameters: countless key pairs can be generated from the same public parameters. This is a very important property.

p is a very large prime number. Its length is recommended to be at least 1024 bits (and must be a multiple of 64 bits). p-1 must be a multiple of q, and q must be 160 bits long. g is the result of a mathematical expression derived from p and q.

The DSA algorithm uses the following parameters:

p: an L-bit prime number. L is a multiple of 64, ranging from 512 to 1024;  
q: a 160-bit prime factor of p – 1;  
g: g = h^((p-1)/q) mod p, where h satisfies h < p – 1 and h^((p-1)/q) mod p > 1;  
x: x < q, where x is the private key;  
y: y = g^x mod p, and ( p, q, g, y ) is the public key;    
...
7. If r and s are greater than q or less than 0, verification fails immediately.
...
10. If r and s are less than 1 or greater than n-1, verification fails immediately.
...
The Rabin scheme is a public-key algorithm designed by M.O. Rabin. It relies on the difficulty of computing square roots modulo N. The Rabin scheme can be used for both public-key encryption and digital signatures.
...
The attack here mainly targets the public key. Authenticating public keys still requires the use of public-key certificates.

### 2. Attacks on One-Way Hash Functions

A collision attack can be performed against a one-way hash function to generate another, different message that has the same hash value as the message bound to the signature.

### 3. Using Digital Signatures to Attack Public-Key Cryptography

Because RSA and digital signatures are inverse operations of each other, this property can be used to attack RSA. If the sender is asked to sign an RSA ciphertext (encrypt it with the private key), that is equivalent to a decryption operation in RSA.

There are several ways to prevent this attack:

- Do not sign arbitrary messages directly; signing the hash value is safer
- Use different key pairs for public-key encryption and digital signatures
- Never sign a message whose meaning is unclear, just as you would not arbitrarily stamp a contract you cannot understand

### 4. Existential Forgery

Even if the object being signed is a meaningless message, such as a random bit string, if an attacker can generate a valid digital signature (that is, the signature generated by the attacker can pass verification normally), this still counts as a potential threat to the signature algorithm. In digital signature algorithms that use RSA to decrypt messages, existential forgery is possible. As long as a random bit string S is encrypted with the RSA public key to produce ciphertext M, then S is a valid digital signature for M. Since the attacker can obtain the public key, existential forgery of the digital signature becomes possible.

To prevent this situation, RSA was improved and a new signature algorithm, RSA-PSS (Probabilistic Signature Scheme), was developed. RSA-PSS does not sign the message itself; instead, it signs the hash value of the message. To improve security, it also salts the message when computing the hash value.

### 5. Other Attacks

Attacks against public-key cryptography can also be used against digital signatures, such as brute-forcing the private key, attempting to factor RSA's N into primes, and so on.

## VIII. Problems Digital Signatures Cannot Solve

The public key used in the public-key cryptography underlying digital signatures must be authenticated separately to prevent man-in-the-middle attacks. The public key used to verify a signature must be certified as belonging to the real sender.

This seems to lead to a circular dependency. Digital signatures are used to detect message tampering, impersonation, and repudiation. But we must obtain an untampered public key from a non-impersonated sender.

To verify that the obtained public key is legitimate, **certificates** must be used. A certificate treats the public key as a message and is obtained by having a trusted third party sign it.

The topic of certificates will be covered in the next article.

## IX. RSA Digital Signatures in OpenSSL

Unlike RSA public-key encryption, RSA-based digital signatures involve digest computation, so a hash algorithm must be specified. The following example uses the sha256 hash algorithm.
```c
// Generate an RSA key pair, key length 1024 bits
...
```c
// Generate the signature file signature.txt for plain.txt using the sha256 hash algorithm and signing algorithm 

$ echo "hello" >> plain.txt
$ openssl dgst -sha256 -sign rsaprivatekey.pem -out signature.txt plain.txt

```
Open the signature.txt file; it contains the result after signing.
```c
...
```c
// Verify the signature file with the same digest and signature algorithms; compare the signature file against the original file
...
Verified OK
```
If you doubt whether the verification above is genuine, we can use another file to validate the signature—for example, switch to a txt file.
```c
...
Verified OK
```
If you suspect the verification above is not genuine, you can validate the signature against a different file—for example, use another txt file—or verify it without PSS mode. Both approaches will fail verification.
```c
// Use another txt file to verify the signature again
...
$ openssl dgst -sha256 -verify rsapublickey.pem -signature signature.txt plain.txt
```
Both of the above methods will fail validation.
...
Generate a key pair
```c
// Generate the key pair dsaprivatekey.pem from the parameter file
...
-----END DSA PRIVATE KEY-----
```
`priv` and `pub` correspond to the private key and public key in a key pair. `P`, `Q`, and `G` are the three key parameters in the parameter file, and they are essential to the DSA algorithm.

Since we can inspect the information in the private key file, we can likewise inspect the information in the public key file.
```c
// View information about the public key and file
...
-----END PUBLIC KEY-----
```
In the output, the four parameters pub, P, Q, and G are consistent with the values output from the private key.
...
Verified OK
```
If you suspect whether the above verification is genuine, you can use another file to verify the signature, such as switching to a txt file.
```c
// Use another txt file to verify the signature again
...
Verified OK
```
If you doubt whether the verification above is genuine, we can use another file to verify the signature—for example, switch to a `.txt` file.
```c
// Replace with another txt file and verify the signature again
...
Verification Failure
```
**DSA signature operations are much slower than RSA signature operations, while ECDSA signature generation is much faster than RSA signature generation; however, ECDSA signature verification is somewhat slower than RSA signature verification**.

Taking both security and performance into account, if choosing between DSA and ECDSA, prefer ECDSA.
