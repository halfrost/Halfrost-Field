...
slug = "tls1-3_start"
title = "This site now supports TLS 1.3"

+++


I’ve recently been studying the TLS 1.3 protocol, so I wanted to get some hands-on experience with it; the blog needs to deploy TLS 1.3 first. Its 0-RTT feature can also improve the blog’s performance. Note that Chrome 68 and Firefox currently support TLS 1.3 draft 28, so do not use it in production environments for now.

More details about TLS 1.3 will be analyzed in later articles.

## I. Install dependencies

First, let’s take a look at the dependencies.

Nginx 1.13.0 officially introduced support for the TLSv1.3 option in ssl\_protocols. Before that, the TLSv1.2 option would automatically enable TLSv1.3.

OpenSSL currently has a draft-18 branch, draft-23 in the pre2 version, and draft-28 in pre7+. You can check the current draft via tls1.h (include/openssl/tls1.h line 35).

TLSv1.3 Draft 23 has been enabled and used by default since Chrome 65, and Draft 28 has been supported since Chrome 68 (Firefox Nightly should support it as well).

My ECS system is centOS 7.0. If you use another distribution, adjust the package-management commands accordingly.

Currently, the latest nginx is Nginx 1.15.3, and the latest OpenSSL is 1.1.1-pre9. This time I will install the latest versions of both.
...
```
Installing the vanilla Nginx 1.15.3 leaves out some useful features. Fortunately, patches are available. I applied 5 patches here and recommend them as well.
...
These patches may not apply to the latest Nginx versions. For tested versions, see [https://github.com/kn007/patch](https://github.com/kn007/patch)
```bash
...
For Nginx with TLS 1.3 0-RTT support, it is best to use version 1.15.4 or later. Here, I upgraded directly to the latest version, 1.15.8.
```bash
...
```
The patch corresponding to Nginx 1.15.8 has changed:
```bash
...
$ cd ..
```
The following is the Strict-SNI Patch; installation is optional, and I did not install it:   
```bash
...
```
Because different versions of Chrome support different TLS variants, we should ideally support all mainstream TLS 1.3 versions. Here we apply another patch to support drafts 23, 26, and 28 simultaneously.
```bash

# Apply TLS 1.3 Draft 23, 26, 28 patches
...
OpenSSL must be version 1.1.1+ to support TLS 1.3 0-RTT; here, I upgraded directly to the latest version, 1.1.1a.
```bash
...
```
Similarly, to support different versions of Chrome, several major draft versions of TLS 1.3 need to be supported: Draft 23, 26, 28, and Final. Patch them in!
```bash

# Apply patches for TLS1.3 Draft 23, 26, 28, and Final Patch

#Depends on the OpenSSL version, see https://github.com/hakasenyang/openssl-patch
$ cd openssl-1.1.1a
...
- Enable OpenSSL to support functionality equivalent to BoringSSL cipher groups.
- In TLS 1.1 and later, prevent 3DES from being used with ECDHE cipher suites to improve security.
...
zlib is recommended by Cloudflare, but I haven’t installed it yet. The main issue is that there seems to be a problem with the compilation parameters, and the resulting build reports errors. I’ll need to look into it further when I have time.
```bash
...
This plugin is also optional to install; I did not install it. This is likewise due to issues with the compilation parameters.
```bash
...
This plugin is also optional; I did not install it. This is likewise due to compilation parameter issues.
```bash
...
All the compilation options together are as follows:
```c
...
```
If you have installed all the patches recommended above, you may run into issues when compiling further. Simply remove `-O2` from the `--with-cc-opt` build option.

My final build options are as follows, for reference only:
```bash
...
```
After updating the build parameters, you can start running make:
```bash
$ make
```
...
Since both Nginx and OpenSSL now support TLS 1.3 0-RTT, the build parameters have also been updated:
```bash
$ cd nginx-1.15.8
...
```
The entries containing TLS13 are Cipher Suites newly added in TLS 1.3; just place them at the beginning.

Finally, run `nginx -t` to test and confirm there are no issues.

## 5. Deploy Nginx

Restart nginx to apply all the configurations above.
```bash
...
### Verifying with Wireshark Packet Capture

If you still want to be sure, you can verify whether TLS 1.3 is enabled by capturing packets.
...
```
I won’t paste the more detailed report here; anyone interested can use
```bash
...
ssl_early_data on;
```
It is also recommended to add the `Early-Data` header to notify the backend and prevent replay attacks.
```bash
proxy_set_header Early-Data $ssl_early_data;
```
Finally, test it with `sudo nginx -t`.
