...
- (NSMutableDictionary *)_componentMethodMapsWithName:(NSString *)name
{
    ...
    NSLog(@"异步方法 = %@ ，config.properties = %@,config.name = %@,,config.clazz = %@,,config.asyncMethods = %@,,config.syncMethods = %@,",dict,config.properties,config.name,config.clazz,config.asyncMethods,config.syncMethods);
