...
第 1 步和第 2 步生成和加密会话密钥，第 3 ~ 6 步压缩和加密消息
...
3. 用公钥密码解密会话密钥，这里是用步骤 3 中生成的接收者的私钥


### 解压和解压缩消息

1. 对步骤 5 中得到经过压缩和加密的消息用对称密码进行解密。使用步骤 6 中生成的会话密钥
2. 对步骤 7 中得到的经过压缩的消息进行解压缩
3. 得到最原始的消息
...
第 1 ~ 4 步恢复发送者发送的散列值，第 5 ~ 8 步对比散列值。
