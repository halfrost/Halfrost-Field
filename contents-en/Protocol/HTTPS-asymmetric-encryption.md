# Exploring Public-Key Cryptographic Algorithms
...
In symmetric encryption, such as the one-time pad, there is the problem of key distribution. DES and AES have the same issue. Because the encryption and decryption keys are the same, the key must be delivered to the recipient. If public-key cryptography is used, there is no need to deliver the decryption key to the recipient, which solves the key distribution problem. Public-key cryptography can be regarded as one of the greatest inventions in the history of cryptography.


## 2. The Key Distribution Problem

To prevent a man-in-the-middle from intercepting the key, the key must be securely delivered to the communication peer. There are four approaches:

### 1. Pre-shared Keys

Although this method is effective, it has limitations. When discussing the one-time pad, we mentioned that hotlines between major powers are encrypted in this way, but the keys are escorted by agents. If the communication peer is nearby, sharing the key in advance is relatively convenient. If the communication peer is located around the world, this approach becomes limited.

In addition, as communication volume grows, the number of keys increases sharply. For pairwise communication among n people, n * (n-1) /2 keys are required. From this perspective, it is also impractical.

### 2. Key Distribution Center

To solve the problem of the increasing number of pre-shared keys, someone proposed the Key Distribution Center (KDC) approach. The key used for each encryption operation is distributed by the key center, and each person only needs to share a key with the key center in advance.

Although this method solves the problem of key proliferation, it introduces new problems.

The key center stores and records all keys. Once it fails or is attacked and compromised, all encryption will come to a halt. This is also a drawback of centralized management.


### 3. Diffie-Hellman Key Exchange

To address the drawbacks of centralized management, key distribution should not be centralized. This led to the Diffie-Hellman key exchange method.

In Diffie-Hellman key exchange, the two parties communicating securely need to exchange some information, and even if an eavesdropper intercepts this information, it does not matter.

Based on the exchanged information, both parties independently generate the same key. The eavesdropper, however, cannot generate the same key. This approach is feasible. However, it is not considered asymmetric encryption, so this article will not discuss it in detail.

### 4. Public-Key Cryptography

Asymmetric encryption has a public key and a private key. The public key can be distributed online; it does not matter if an eavesdropper obtains it, because without the private key, they cannot decrypt the ciphertext. As long as the private key is held only by the recipient, the ciphertext will not be compromised.

For example: consider the baggage lockers in a supermarket. Any customer with a coin can store a bag. The coin is the “public key”. The customer puts the bag into the locker (encrypting the plaintext), and once the locker is locked, no one can open it. At this point, an eavesdropper cannot take the stored bag either. This plaintext (the bag) can only be opened with the private key. After the customer stores the bag, a private key is generated; as long as they have this key, they can open the locker and retrieve the bag at any time.


## 3. Asymmetric Encryption

Asymmetric encryption generally refers to encryption algorithms based on public-key cryptography. Keys are divided into two types: encryption keys and decryption keys. The sender encrypts the message with the encryption key, and the recipient decrypts the ciphertext with the decryption key. The key that can be made public is called the public key, while the key kept private and not disclosed is called the private key.

A public key and a private key correspond one-to-one. A pair consisting of a public key and a private key is collectively called a key pair. Mathematically, the two cannot be generated independently.


## 4. Problems with Asymmetric Encryption
...
RSA is a public-key cryptographic algorithm. Its name is formed from the initials of the surnames of its three developers: Ron Rivest, Adi Shamir, and Leonard Adleman (Rivest-Shamir-Adleman). In 1983, RSA obtained rights to the RSA algorithm in the United States, but the patent has now expired.

RSA can be used for public-key encryption and digital signatures.

### 1. RSA Encryption

In RSA, the plaintext, key, and ciphertext are all numbers. The encryption process can be expressed by the following formula:
```c
Ciphertext = Plaintext^E mod N
```
RSA ciphertext is the result of raising the number representing the plaintext to the power of E and taking mod N. E and N are the RSA encryption key; **the combination of E and N is the public key**. E is the initial letter of Encryption, and N is the initial letter of Number.
...
```
The decryption process computes the ciphertext number raised to the power of D, mod N, to obtain the plaintext. **The combination of D and N is the private key**. D is the initial of Decryption, and N is the initial of Number.

What is remarkable about RSA is that the encryption and decryption processes are consistent. Encryption computes the plaintext raised to the power of E, modulo N. Decryption computes the ciphertext raised to the power of D, modulo N.
...
E and N form the public key, while D and N form the private key. Therefore, computing the three values E, D, and N is the process of **generating the key pair**. The specific workflow consists mainly of four steps:

- Compute N
- Compute L (L is a number used only during key-pair generation)
- Compute E
- Compute D

The main steps are shown in the following diagram:
...
To compute N, you need to prepare two very large prime numbers. If the primes are too small, the key is easy to break; if they are too large, computation takes a very long time. Both p and q are generated by a pseudorandom number generator, and p != q. N = p * q.

Next, compute L.

The number L is used only during key-pair generation; it does not appear during either encryption or decryption.
L is the least common multiple (least common multiple, lcm) of p - 1 and q - 1.

Next, compute the number E.
...
```
Condition 1 is generated using a pseudorandom spanning tree. Condition 2 ensures that the number D required for decryption definitely exists. The greatest common divisor can be computed using Euclid’s algorithm.
...
E * D mod L = 1
```
As long as D satisfies the conditions above, ciphertext encrypted with E and N can be decrypted with D and N. To ensure that a D satisfying the conditions definitely exists, we must first ensure that the greatest common divisor of E and L is 1. The condition `E * D mod L = 1` guarantees that the plaintext can be recovered after decrypting the ciphertext.
...
E * D mod L = 1 ensures the final transformation can be restored to plaintext. For a detailed derivation, see Teacher Ruan's two RSA analysis articles in the Reference section at the end of the article.
```

## 6. Directly Attacking RSA

To directly attack RSA, that is, to derive the plaintext from the ciphertext. We know:
```c
Ciphertext = Plaintext^E mod N
```
An attacker can obtain the ciphertext and the public key `{E,N}`. The problem is then transformed into finding a discrete logarithm. There is currently no efficient algorithm for this problem, so this approach is infeasible.

Let's consider another approach: compute the private key, then recover the plaintext sequentially by decrypting. The public key contains `N`, and the private key also requires `D`. So can `D` be brute-forced?

In general, `p` and `q` are both over 1024 bits long, and `N` is over 2048 bits long. The length of `D` is roughly the same as that of `N`, so this would require brute-forcing more than 2048 bits, which is extremely difficult within any realistic time frame. (Unless quantum computers appear in the future; of course, if quantum computers do appear, all existing encryption systems today would be completely upended.)

Since brute-forcing `D` is impossible, is it feasible to compute `D` from `E` and `N`? To summarize, the available conditions for brute-forcing `D` are:
```c
E * D mod L = 1
...
```
At this point, we know E and N. To compute D, we first need to compute L, and L is related to p and q. Moreover, p * q happens to equal N. So our first step is to crack p and q.

At present, factoring a large number N into its prime factors is difficult. **Once an efficient algorithm for prime-factorizing large integers is discovered, RSA can also be broken**. No effective algorithm exists yet. It is also impossible to derive p and q by guessing, because p and q are typically very long.
...
The active attacker Mallory intercepts the public keys of both communicating parties, and sends each party her own public key. In this way, the attacker can intercept the ciphertext from both sides and decrypt it with her own private key. After decrypting the ciphertext to obtain the plaintext, she can arbitrarily tamper with the original message, ultimately achieving the attack objective.


This kind of attack cannot be defended against by RSA itself alone. To defend against man-in-the-middle attacks, authentication-related algorithms are required. Authentication-related algorithms will be explained in detail in the next article.


## VIII. Elliptic Curve Cryptography

<p align='center'>
<img src='https://img.halfrost.com/Blog/ArticleImage/101_16.webp'>
</p>

Elliptic Curve Cryptography (ECC) is a public-key cryptographic algorithm that has received significant attention in recent years. Its key characteristic is that it requires a shorter key length than RSA. **Elliptic curve cryptography uses short keys while providing high strength**. Elliptic curve cryptography with a key length of 224–255 bits provides the same security strength as RSA with a key length of 2048 bits.
...
Because ECC keys are very short, operations are relatively fast. So far, performing the inverse operation for ECC remains very difficult; mathematically, it is proven to be unbreakable. The advantages of ECC are high performance and strong security. In practical applications, it can be combined with other public-key algorithms to form faster and more secure public-key algorithms. For example, combining it with DH key exchange yields the ECDH key agreement algorithm, and combining it with the digital signature algorithm DSA yields the ECDSA digital signature algorithm.
...
- Key exchange based on elliptic curves
...
The above is an elliptic curve, but it does not look like an ellipse.

### 1. Operations on Elliptic Curves

Addition: Given two points A and B on an elliptic curve, draw the line through them. The reflection across the X-axis of the line’s intersection with the elliptic curve is defined as A+B. As shown below:
...
Of course, the two points may also coincide. In this case, it is equivalent to finding the doubled point. For a point A on an elliptic curve, draw the tangent line at A. The reflection across the X-axis of the tangent’s other intersection with the elliptic curve is the doubled point. This operation is called point doubling, as shown below:

<p align='center'>
<img src='https://img.halfrost.com/Blog/ArticleImage/101_10_.png'>
</p>

The point symmetric to point A with respect to the X-axis is called -A. This operation is called negation on an elliptic curve. As shown below:
...
The essence of elliptic curve cryptography is the use of the Elliptic Curve Discrete Logarithm Problem (ECDLP). In essence, it is **the problem of finding the number x given the point xG**.
...
Over the real numbers, an elliptic curve is continuous, and the curve is smooth. Suppose the elliptic curve is E2: y^2 = x^3 + x + 1, as shown below:
...
For every point in the figure above, its y-coordinate modulo 23 is equal to x^3 + x + 1 modulo 23. If we take the point G=(0,1) on E2 as the base point, then we can compute 2G, 3G, 4G, 5G, and so on according to the computation rules of elliptic curves.
...
However, elliptic curve cryptographic algorithms are not defined over the real-number field R, but over **a finite field F(P)**. A finite field F(P) refers to the addition, subtraction, multiplication, and division operations defined on the set of p integers 0, 1, 2, …, p-1 for some given prime number p.

>In addition to using the prime field GF(P) with characteristic p, elliptic curve cryptographic algorithms can also use the extension field GF(2^m) with characteristic 2^m.
...
>In the 1990s, symmetric-key cryptography became more mature, with the emergence of Rijndael, RC6, elliptic curves, and other public-key algorithms  
...
- In the DH algorithm, during **key agreement**, neither communicating party can independently compute a session key. Each party keeps part of the critical information and tells the other party the other part. Only after both parties have all the information can they jointly compute the complete session key.
- In the RSA algorithm, when transmitting the session key, the session key is generated and controlled entirely by the client, without participation from the server. Strictly speaking, it should be called **RSA key transport**.


Here is an example to illustrate how the ECDH algorithm generates a shared key.
...
- Alice generates a random number a. This number is **known only to Alice herself**. There is no need to tell Bob, and it must not be known by any third party. This a is called Alice’s private key.
- Bob generates a random number b. This number is **known only to Bob himself**. There is no need to tell Alice, and it must not be known by any third party. This b is called Bob’s private key.
...
An eavesdropper can obtain three pieces of useful information: G, aG, and bG. However, because “it is very hard to find x given G and xG,” knowing G and aG does not allow one to solve for a, and knowing G and bG does not allow one to solve for b. Therefore, the private key abG cannot ultimately be derived.

>Strictly speaking, the complexity of the discrete logarithm problem on elliptic curves can only prove that “given G, aG, and bG, it is hard to find a and b,” but it cannot prove that “given G, aG, and bG, it is hard to find abG.” The latter requires a separate proof, which is omitted here.

Combining the static DH algorithm with ECC gives the ECDH algorithm. This approach uses the same base point G every time. Its advantage is that it avoids having the server frequently generate G each time a connection is initialized. That process consumes a relatively large amount of CPU. However, its drawback is that once the random numbers a and b are leaked, all previous sessions can be decrypted.

To solve this problem, the DHE algorithm (Diffie-Hellman Ephemeral, an ephemeral DH algorithm) was introduced, and when combined with ECC it forms the ECDHE algorithm. It can ensure that the shared key used for each communication session is different. The DH key pair exists only in memory, unlike an RSA private key, which is stored on disk. Even if an attacker extracts the private key from memory, only the current communication session is affected, so there is no need to worry that previous communications will be decrypted. This property is called Forward Secrecy (FS) or Perfect Forward Secrecy (PFS). Even more securely, after the session key is negotiated, the two private keys a and b can be discarded, further improving security: the key pair is generated within limited time and effective space. The ECDHE\_ECDSA and ECDHE\_RSA key exchange algorithms are used in the TLS handshake.
...
Suppose Alice wants to attach a digital signature to message m, and Bob needs to verify that signature. All “computations” below refer to clock arithmetic modulo p.
...
Suppose Alice wants to send a message to Bob. Alice can represent the message she wants to send as a point M on the elliptic curve (in practice, using the x-coordinate of that point).
...
This again relies on the property that an eavesdropper cannot derive abG, thereby ensuring the security of the ciphertext.


## IX. Other Public-Key Encryption Algorithms

The ElGamal scheme is a public-key algorithm designed by Taher ElGamal. RSA relies on the difficulty of prime factorization, while ElGamal relies on the difficulty of computing discrete logarithms modulo N.
...
Using public-key cryptography can solve the key distribution problem, but public-key cryptography exposes two new problems.
...
The first drawback can be solved by the hybrid cryptosystem discussed in this section. The second drawback requires the authentication-related algorithms discussed in the next article.

A hybrid crytosystem is a method that combines the advantages of symmetric cryptography and public-key cryptography. Fast symmetric cryptography is used to encrypt the plaintext, and public-key cryptography is used to encrypt the symmetric key. Since a symmetric key is generally much shorter than the message itself, the slow speed of public-key cryptography can be ignored.

- Encrypt the plaintext message with symmetric cryptography
- Generate the session key used for symmetric encryption with a pseudorandom generator
- Encrypt the session key with public-key cryptography
- Provide the key used for public-key encryption from outside the hybrid cryptosystem

A session key is a temporary key generated for the current communication session. It is generally produced by a pseudorandom number generator. The session key produced by the pseudorandom generator is also used as the key for the symmetric cipher. **The session key is the key for the symmetric cipher, and it is also the plaintext for the public-key cipher**.

The following figure shows the encryption flow in a hybrid cryptosystem:

<p align='center'>
<img src='https://img.halfrost.com/Blog/ArticleImage/101_22.png'>
</p>

It is worth noting that the strength of the public-key cryptosystem should be higher than that of the symmetric cryptosystem. This is because if the symmetric session key is broken, only the local communication content is affected; but if the public key’s private key is broken, all past communications and all future communications will be affected.

The following figure shows the decryption flow in a hybrid cryptosystem:
...
Use the genrsa subcommand to generate a key pair. The key pair is a single file.
```c
...
```c
// Use a passphrase with the 3DES algorithm to protect the key pair
...
```c
// The -noout option means not to print key pair information; if the check succeeds, the key pair file is valid
...
-----END PUBLIC KEY-----
```
Modulus is N in the RSA encryption structure. Exponent is E in the public key. The content between -----BEGIN PUBLIC KEY----- and -----END PUBLIC KEY----- is the actual public key value.

Encrypt using a key pair
```c
// The default padding mechanism for the rsautl command is PKCS#1 v1.5
$ openssl rsautl -encrypt -inkey mykey.pem -in plain.txt -out cipher.txt

// Specify the rsautl padding mechanism as PKCS#1 OAEP
$ openssl rsautl -encrypt -inkey mykey.pem -in plain.txt -out cipher.txt -oaep
```
Suppose the plaintext in the file we encrypted is "hello world". After encryption, the ciphertext becomes:
```c
�1Au�&.�rzC��7��6:
+��٠P�?��|��~İ���}'x�������X�IkBx��V2��~�/��$�N�(5m�s�#�m�0����s�ɜ+jėއ(!E�t��??�{���Y�W�a��tȈp���uUlzk�I9W����[���/�l!J��ө��-v�O_h����b~ ��Y
```
When encrypting with a public key, be sure to include the `-pubin` option to indicate that the `-inkey` parameter specifies a public key file.
```c
$ openssl rsautl -encrypt -pubin -inkey mypubkey.pem -in plain.txt -out cipher2.txt
```
The plaintext encrypted in the same way is "hello world", and after encryption the ciphertext becomes:
```c
�
...
Unlike RSA key-pair generation, DH key-pair generation is divided into two steps. The first step is to generate a parameter file, and the second step is to generate a key pair based on that parameter file. **The same parameter file can be used to generate an unlimited number of unique key pairs**.

Step 1: Generate a DH key pair
```c
...
// View the key pair file contents
...
DH Private-Key: (2048 bit)
    ...
    generator: 2 (0x2)
```
`prime` is a large prime number, and `generator` is the generator.

Next, let’s walk through a complete example of DH key agreement.
```c
// One party generates the DH parameter file; it can be made public
...
DH Parameters: (1024 bit)
    ...
    generator: 2 (0x2)
```
Sender Alice generates a key pair based on the parameter file.
```c
...
-----END PUBLIC KEY-----
```
Bob extracts the public key file bkey\_pub.pem and keeps the private key himself.
```c
...
OpenSSL supports many named curves; let’s first look at which curves are supported.
```c
...
```
Generate a parameter file and specify the named curve with `-name`.
```c
...
```c
// Show the specific parameters in the parameter file
...
- A and B are parameters in the elliptic curve equation
- P is a large prime that constrains the size of all ECC points within a finite field. Almost all ECC operations perform modulo operations with respect to P
- Generator is the base point G, composed of (gx,gy)
- Order corresponds to the base point’s n, i.e., the number of times the base point can be added to itself before returning to the identity. Most ECC operations do not need this value, except that ECDSA takes the signature computation modulo Order rather than modulo P
- Cofactor is equal to the total number of points on the elliptic curve divided by n.

>In ECC, k is the private key, g is the base point, and kg is computed from the curve formula to obtain the public key. It is very difficult to derive the private key k from the public key; this is backed by complex mathematical theory, namely the discrete logarithm problem.
