...
**Concept.** Just as `EngineCoreClient` ([Section 4](#4-the-engine-boundary-in-process-vs-the-multiprocess-client)) hides whether the engine core is in-process or behind a ZMQ socket, the `Executor` hides whether the model runs on one GPU or many. The engine core never addresses a worker; it names an *operation* and a *target set*, and lets the executor broadcast.
...
**Concept.** [Section 6](#6-the-engine-step-schedule-execute-process-outputs) showed `EngineCore.step()` calling `execute_model`, then — only if the result is `None` — calling `sample_tokens(grammar_output)`. That `None` is a deliberate signal from the worker: "I ran the forward pass and stashed the hidden states; call me back to sample."
...
**The design.** Sampling is a distinct, resumable phase keyed to `execute_model_state`. A request can be aborted, or a grammar mask computed, in the window between the forward pass finishing and tokens being drawn — which is precisely the abort window [Section 6](#6-the-engine-step-schedule-execute-process-outputs) identified between `future.result()` and `update_from_output`.

### What comes back

`sample_tokens` packages the drawn token IDs into a `ModelRunnerOutput`, and the executor returns the authoritative worker's copy up to `EngineCore.step()` as `model_output`. From there [Section 6](#6-the-engine-step-schedule-execute-process-outputs) takes over: `update_from_output` folds those tokens into request progress and frees finished KV blocks. Note one detail that matters downstream — a single step's `ModelRunnerOutput` may carry **more than one** new token per request (speculative decoding, multi-token prediction), so "one step, one token" is the common case, not an invariant.
...
Concept: the sampled id does not return to the caller as an integer. It rides back as a field of the wire-shaped `EngineCoreOutput`, folded into request progress by `update_from_output` ([Section 6](#6-the-engine-step-schedule-execute-process-outputs)).
...
The design: **online and offline share one code path and diverge at a single branch; the streaming mailbox internals and the async generator live in [Section 11](#11-streaming-the-async-generator-and-why-the-first-token-is-special), and the deep async fan-out lives in article 04.** What this prevents: two parallel output-processing implementations drifting apart — the same stop-string, delta, and parallel-sampling semantics are guaranteed identical for `LLM.generate()` and the OpenAI server because they run the same loop.

### What to carry forward

The output processor is where token IDs re-acquire meaning. It owns four things the engine deliberately does not: **incremental detokenization** (IDs → text, with token/text divergence and `min_tokens` respected), **stop-string detection** (the one signal that flows *back* into the engine, via `reqs_to_abort`), **output-kind gating** (FINAL_ONLY / DELTA / CUMULATIVE / stream_interval, deciding whether a step surfaces anything at all), and **identity** (always the external id). It is one race-safe loop, and its single `queue is not None` fork is the seam between blocking offline collection ([Section 2](#2-llmgenerate-the-offline-convenience-layer)) and streaming online delivery ([Section 11](#11-streaming-the-async-generator-and-why-the-first-token-is-special)). Hand-offs: **article 10** for logprobs assembly and the detokenizer backends (Rust `DecodeStream` fast path vs. Python slow path); **article 04** for the async `output_handler` fan-out and finished-request lifecycle inside the engine; **[Section 11](#11-streaming-the-async-generator-and-why-the-first-token-is-special)** for the `RequestOutputCollector` mailbox and how the first `RequestOutput` becomes the first token a client sees.
...
## 12. The Full End-to-End Trace, and What to Remember
